        ...
        <div class="card">
            <div class="card-body">
                <div class="card-edit-pegawai">
                    ...
                    <NuxtLink class="btn btn-primary" to="/data_pengajuan_cuti_sekretaris">Kembali</NuxtLink>
                </div>
...
export default {
    ...
    methods: {
            ...
            try {
                ...
                if (result.isConfirmed) {
                    if (!this.rememberMe) {
                        ...
                        try {
                            const accessToken = localStorage.getItem('token');
                            const response = await axios.put(`http://127.0.0.1:8000/api/pengajuan_cuti_penyetuju/${cutiId}`, requestData, {
                                headers: {
                                    'Authorization': `Bearer ${accessToken}`
                                }
                            });
        ...
        backDataPengajuanCuti() {
            this.$router.push('/data_pengajuan_cuti_sekretaris');
        },
    }
}
</script>
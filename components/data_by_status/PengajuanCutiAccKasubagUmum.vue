  ...
  methods: {
          ...
          .then(res => {
            ...
            if (res.data.data !== null) {
              ...
              if (res.data.data.length === 0) {
                ...
                this.data_cuti = res.data.data.filter(cuti => cuti.user_id !== idUser && (cuti.status === "Selesai" || cuti.status === "ACC Kabid" || cuti.status === "ACC Kasubag Umum" || cuti.status === "Ditolak Kasubag Umum"));
              }
    ...
    getDataPengajuanCuti() {
      ...
      axios.get('http://127.0.0.1:8000/api/pengajuan_cuti_acc_kasubag', {
        ...
        console.log(res.data.data);
        this.data_cuti = res.data.data.filter(cuti => cuti.user_id !== idUser && (cuti.user.role !== 'kasubag' && cuti.user.role !== 'sekretaris' && cuti.user.role !== 'kadis'));
      }).catch(error => {
        console.error('Error fetching data:', error);
      });
    },
    ...
    async validasi(cutiId) {
      try {
        ...
        if (result.isConfirmed) {
          if (!this.rememberMe) {
            const accessToken = localStorage.getItem('token');
            await axios.put(`http://127.0.0.1:8000/api/pengajuan_cuti_acc_kasubag/${cutiId}`, null, {
              headers: {
                'Authorization': `Bearer ${accessToken}`
              }
            });
    ...
    async validasiTolak(cutiId) {
      try {
        ...
        if (result.isConfirmed) {
          if (!this.rememberMe) {
            const accessToken = localStorage.getItem('token');
            await axios.put(`http://127.0.0.1:8000/api/pengajuan_cuti_tolak_kasubag/${cutiId}`, null, {
              headers: {
                'Authorization': `Bearer ${accessToken}`
              }
            });
